...
def preprocess_keypoints_for_model(keypoints, max_length):
    ...
    keypoints_padded = pad_sequences([keypoints_flattened], maxlen=max_length * 34, padding='post', dtype='float32')
...
def process_image(file_path, folder_name, file_name, fall_type):
    ...
    if results[0].keypoints is None or results[0].keypoints.xy.numel() == 0:
        print(f"No keypoints detected for image {file_name}. Skipping.")
        return

    keypoints = results[0].keypoints.xy[0].cpu().numpy()  # Ensure keypoints are in the right format
    ...
    output_image_path = f'processed_images/{file_name}'
...
for index, row in df.iterrows():
    ...
    process_image(file_path, folder_name, file_name, fall_type)

df = add_angles_to_df(df)

df.to_csv(output_csv_file, index=False)

print(f"Processing completed. Output saved to {output_csv_file}.")
